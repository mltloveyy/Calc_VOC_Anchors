...
void writeXmlFile(std::string xml_file, std::string img_name, xmlReadWrite &img_info)
{
	...
	for (int i = 0; i < img_info.xml_object_vec.size(); i++)
	{
		...
		if (img_info.xml_object_vec[i].gt_shapes.size() > 0)
		{
			...
			ShapeElement->SetAttribute("thickness", "7");
...
std::pair<cv::Mat, xmlReadWrite> img_Blur_Kernel(cv::Mat input_img, xmlReadWrite input_xml, int blur_kernel_size, float sigma)
{
	// 存储结果
	cv::Mat output_img;
	xmlReadWrite output_xml;
	...
	cv::GaussianBlur(input_img, output_img, cv::Size(blur_kernel_size, blur_kernel_size), sigma);
...
std::pair<cv::Mat, xmlReadWrite> img_GaussianNoise(cv::Mat input_img, xmlReadWrite input_xml)
{
	...
	std::random_device r;
	default_random_engine e1(r());
	uniform_int_distribution<unsigned> u1(3, 10);
...
std::pair<cv::Mat, xmlReadWrite> img_Illum_lighting(cv::Mat input_img, xmlReadWrite input_xml, cv::Point center, float power_min, float power_max, float scale_min, float scale_max, std::string light_type)
{
	// 存储结果
	cv::Mat output_img;
	xmlReadWrite output_xml;
	...
	uniform_int_distribution<unsigned> u0_x(-800, 800);
	uniform_int_distribution<unsigned> u0_y(-5, 5);
	uniform_int_distribution<unsigned> u1(power_min, power_max);
	uniform_int_distribution<unsigned> u2(scale_min, scale_max);

	cv::Point value_center(center.x + u0_x(e1), center.y + u0_y(e1));
...
std::pair<cv::Mat, xmlReadWrite> img_Rotate(cv::Mat input_img, xmlReadWrite input_xml, int rotate_angle_start, int rotate_angle_end)
{
	...
	uniform_int_distribution<unsigned> u0(0, 100);
	uniform_int_distribution<unsigned> u1(rotate_angle_start, rotate_angle_end);
	uniform_int_distribution<unsigned> u2(0, 1);

	int center_x = 0.5*input_img.cols;
	int center_y = 0.5*input_img.rows;

	cv::Point2f rotate_center(center_x + u0(e1) - 50, center_y + u0(e1) - 50);

	int flag_rotate = u2(e1) > 0 ? 1 : -1;
	...
	for (auto idx_obj = 0; idx_obj < input_xml.xml_object_vec.size(); idx_obj++)
	{
		...
		for (auto idx_shape = 0; idx_shape < input_xml.xml_object_vec[idx_obj].gt_shapes.size(); idx_shape++)
		{
			...
			if (point.x > output_img.cols - 1) {
				flag_out = true;
				point.x = output_img.cols - 1;
			}
			if (point.y < 0) {
				flag_out = true;
				point.y = 0;
			}
			if (point.y > output_img.rows - 1) {
				flag_out = true;
				point.y = output_img.rows - 1;
			}
			if (flag_out) {
				out_shapes++;
			}
...
// 添加pad截取
void boxAug1(std::pair<cv::Mat, xmlReadWrite> img_aug, struct augParameter aug_parameter, std::vector<cv::Mat> &img_rois)
{
	cv::Mat img = img_aug.first.clone();
	xmlReadWrite annotation = img_aug.second;

	int num_rotate_angle = int(360 / aug_parameter.scale_angle);  // 旋转扰动次数
	int num_box_in_an_angle = int(aug_parameter.num_aug_max / num_rotate_angle);  // 每次旋转扰动个数
	num_box_in_an_angle = num_box_in_an_angle > 1 ? num_box_in_an_angle : 1;  // 每次旋转至少扰动一次
	...
	for (int idx_obj = 0; idx_obj < annotation.xml_object_vec.size(); idx_obj++)
	{
		...
		if (aug_parameter.flag_aug)
		{
			std::random_device r;
			default_random_engine e1(r());
			uniform_int_distribution<unsigned> u_scale(aug_parameter.scale_min * 100, aug_parameter.scale_max * 100);
			...
			for (int idx_angle = 0; idx_angle < num_rotate_angle; idx_angle++)
			{
				...
				do
				{
					float img_scale_x = u_scale(e1) / 100.0;  // x scale
					float img_scale_y = u_scale(e1) / 100.0;  // y scale
					float value_angle = aug_parameter.scale_angle * idx_angle + u_angle(e1);  //rotate angle

					int radius_w = u_center(e1) / 100.0 * obj_box.width;  // elips x-side
					int radius_h = u_center(e1) / 100.0 * obj_box.height;  // elips y-sdie

																																 // 根据角度θ，求椭圆上的点到圆心的距离：w × h / √((wcosθ)²+(hsinθ)²)
					int radius_elips = radius_w*radius_h*std::sqrt(1.0 / ((radius_w*radius_w*std::pow(std::cos(value_angle), 2)) + (radius_h*radius_h*std::pow(std::sin(value_angle), 2))));
					uniform_int_distribution<unsigned> u_radius(0, radius_elips);
					int random_radius = u_radius(e1);

					int new_centerx = obj_box.x + obj_box.width*0.5 + random_radius*std::cos(value_angle);
					int new_centery = obj_box.y + obj_box.height*0.5 + random_radius*std::sin(value_angle);
					int new_w = obj_box.width * img_scale_x;
					int new_h = obj_box.height * img_scale_y;
					cv::Rect new_box = cv::Rect(new_centerx - 0.5*new_w, new_centery - 0.5*new_h, new_w, new_h);
					new_box &= cv::Rect(0, 0, img.cols, img.rows);

					// 剔除异常长宽比
					float max_ratio_old = 1.0*max(obj_box.width, obj_box.height) / min(obj_box.width, obj_box.height);
					float max_ratio_new = 1.0*max(new_box.width, new_box.height) / min(new_box.width, new_box.height);
					...
					// 计算前后框的iou，box-iou:交集/并集；shape-iou：交集/生成框
					float iou = 0.0;
					...
					if (iou > aug_parameter.value_iou)
					{
						...
						int left_addpad = max(new_box.x - aug_parameter.width_pad, 0);
						int right_addpad = min(new_box.x + new_box.width + aug_parameter.width_pad, img.cols - 1);
						int top_addpad = max(new_box.y - aug_parameter.height_pad, 0);
						int bottom_addpad = min(new_box.y + new_box.height + aug_parameter.height_pad, img.rows - 1);
						new_box.x = left_addpad;
						new_box.y = top_addpad;
						new_box.width = right_addpad - left_addpad;
						new_box.height = bottom_addpad - top_addpad;
		...
		//// 保存原始框
		//int left_addpad = max(obj_box.x - aug_parameter.width_pad, 0);
		//int right_addpad = min(obj_box.x + obj_box.width + aug_parameter.width_pad, img.cols - 1);
		//int top_addpad = max(obj_box.y - aug_parameter.height_pad, 0);
		//int bottom_addpad = min(obj_box.y + obj_box.height + aug_parameter.height_pad, img.rows - 1);
		//obj_box.x = left_addpad;
		//obj_box.y = top_addpad;
		//obj_box.width = right_addpad - left_addpad;
		//obj_box.height = bottom_addpad - top_addpad;
...
// 固定长宽截取
void boxAug2(std::pair<cv::Mat, xmlReadWrite> img_aug, struct augParameter aug_parameter, std::vector<cv::Mat> &img_rois)
{
	cv::Mat img = img_aug.first.clone();
	xmlReadWrite annotation = img_aug.second;
		...
		if (aug_parameter.flag_aug)
		{
			...
			for (int idx_angle = 0; idx_angle < num_rotate_angle; idx_angle++)
			{
				...
				do
				{
					...
					int new_centerx = obj_box.x + obj_box.width*0.5 + random_radius*std::cos(value_angle);
					int new_centery = obj_box.y + obj_box.height*0.5 + random_radius*std::sin(value_angle);
					int new_w = aug_parameter.width_fix;
					int new_h = aug_parameter.height_fix;
					int new_x = max(0, new_centerx - 0.5*new_w);
					new_x = min(new_x, img.cols - new_w - 1);
					int new_y = max(0, new_centery - 0.5*new_h);
					new_y = min(new_y, img.rows - new_h - 1);
					cv::Rect new_box = cv::Rect(new_x, new_y, new_w, new_h);

					num_box_generate_in_this_angle++;
					idx_boxes_generate.push_back(new_box);
				} while ((num_box_generate_in_this_angle < num_box_in_an_angle));
			}
		}

		// 保存原始框
		int center_x = obj_box.x + obj_box.width*0.5;
		int center_y = obj_box.y + obj_box.height*0.5;
		if (center_y < aug_parameter.width_fix / 4 || center_y > img.rows - aug_parameter.width_fix / 4)
		{
			continue;
		}
		int new_w = aug_parameter.width_fix;
		int new_h = aug_parameter.height_fix;
		int new_x = max(0, center_x - 0.5*new_w);
		new_x = min(new_x, img.cols - new_w - 1);
		int new_y = max(0, center_y - 0.5*new_h);
		new_y = min(new_y, img.rows - new_h - 1);
		obj_box.x = new_x;
		obj_box.y = new_y;
		obj_box.width = aug_parameter.width_fix;
		obj_box.height = aug_parameter.height_fix;
...
void CropPatches(std::pair<cv::Mat, xmlReadWrite> img_aug, struct augParameter aug_parameter, std::vector<cv::Mat>& patches, bool keep_ratio)
{
	cv::Mat img = img_aug.first.clone();
	xmlReadWrite annotation = img_aug.second;
	...
	if (keep_ratio)
	{
		boxAug2(img_aug, aug_parameter, img_rois);
	}
	else
	{
		boxAug1(img_aug, aug_parameter, img_rois);
	}
	...
	//	if (keep_ratio)
	//	{
	//		boxAug2(img_aug_affine, aug_parameter, img_rois);
	//		boxAug2(img_aug_rotate, aug_parameter, img_rois);
	//	}
	//	else
	//	{
	//		boxAug1(img_aug_affine, aug_parameter, img_rois);
	//		boxAug2(img_aug_rotate, aug_parameter, img_rois);
	//	}
	//}

	// stage3:对stage1和stage2生成的扰动框进行翻转、噪声、模糊...等操作
	int box_fuse_ratio = (int)aug_parameter.value_fuse*img_rois.size();
	if (box_fuse_ratio > 0)
	{
		std::random_shuffle(img_rois.begin(), img_rois.end());
	}
	...
	for (int idx_roi = 0; idx_roi < img_rois.size(); idx_roi++)
	{
		...
		{		
			...
			std::random_device r;
			default_random_engine e1(r());
			uniform_int_distribution<unsigned> u_choose(1, flags_aug_rois.size() - 1);
			int aug_choose = u_choose(e1);
			for (int idx_choose = 0; idx_choose < aug_choose; idx_choose++)
			{
				img_crop = imgAug(img_crop, flags_aug_rois[idx_choose]);
			}
		}

		if ((aug_parameter.height_resize) && (aug_parameter.width_resize))
		{
			cv::resize(img_crop, img_crop, cv::Size(aug_parameter.width_resize, aug_parameter.height_resize), 0, 0, cv::INTER_AREA);
		}

		patches.push_back(img_crop);
	}
}
