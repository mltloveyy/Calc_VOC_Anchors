...
// 切正类：调小中心点移动范围、调大iou阈值
// 切负类：调大中心点移动范围、调小iou阈值
struct augParameter
{
	int width_resize = 0;			  // 最终裁剪resize的尺度，置0关闭
	int height_resize = 0;			// 最终裁剪resize的尺度，置0关闭
	int width_fix = 576;				// 固定裁剪尺度
	int height_fix = 576;				// 固定裁剪尺度
	int width_pad = 50;					// 对扰动框加pad
	int height_pad = 150;				// 对扰动框加pad
	int num_aug_min = 6;				// 原始box生成最少的box数目
	int num_aug_max = 6;				// 原始box生成最多的box数目
	int ratio_max = 2.0;			  // 新框长宽比不超过原始box的倍数
	float scale_min = 0.8;			// 原始box缩放最小值
	float scale_max = 1.2;			// 原始box缩放最大值
	float scale_min_r = 0.05;		// 中心点移动最小范围
	float scale_max_r = 0.2;		// 中心点移动最大范围
	float scale_angle = 60;			// 间隔旋转角度
	float value_iou = 0.8;			// iou阈值
	float value_zoom = 2.0;			// 单边最大放大尺度
	float value_fuse = 0;				// 复合扰动选取的比例，置0关闭
	bool flag_aug = true;				// 扰动开关
	bool flag_overlap = false;	// false: 根据box计算iou; true:根据shape点计算iou
...
void readFilesRecurse(const std::string &path, std::vector<std::string> &files_name, std::vector<std::string> &files_name_full, int layer = 10, std::string img_ext = "");
...
std::pair<cv::Mat, xmlReadWrite> img_Blur_Kernel(cv::Mat input_img, xmlReadWrite input_xml, int blur_kernel_size = 3, float sigma = 2.0);
...
std::pair<cv::Mat, xmlReadWrite> img_Illum_lighting(cv::Mat input_img, xmlReadWrite input_xml, cv::Point center, float power_min = 90, float power_max = 110, float scale_min = 70, float scale_max=150, std::string light_type = "gaussian");
